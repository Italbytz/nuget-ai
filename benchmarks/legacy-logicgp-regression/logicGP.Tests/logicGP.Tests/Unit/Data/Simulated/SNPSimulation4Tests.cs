...
[TestClass]
public sealed class SNPSimulation4Tests
{
    [TestMethod]
    public void TestSimulation4RlRw()
    {
        var trainer =
            new LogicGpRlcwMacroMulticlassTrainer<BinaryClassificationOutput>(-1,
                300,
                folds: 5, minMaxWeight: 1.05);

        GPASSimulation("Simulation4", AppDomain.CurrentDomain.BaseDirectory,
            trainer);
        ...
        IEstimator<ITransformer> trainer)
    {
        ...
        for (var j = 1; j < 101; j++)
        {
            ...
            var trainData =
                mlContext.Data.LoadFromTextFile<SNPEnvironmentModelInput>(
                    trainDataPath,
                    ',', true);
            ...
            var pipeline = mlContext.Transforms.ReplaceMissingValues(new[]
                {
                    ...
                    new InputOutputColumnPair(@"E1", @"E1"),
                    new InputOutputColumnPair(@"E2", @"E2")
                }).Append(mlContext.Transforms.NormalizeBinning(new[]
                    {
                        new InputOutputColumnPair(@"E1",
                            @"E1"),
                        new InputOutputColumnPair("E2",
                            "E2")
                    }, maximumBinCount: 4)
                    .Append(mlContext.Transforms.Concatenate(@"Features",
                        ...
                        @"SNP8",
                        @"SNP9", @"SNP10", @"SNP11", @"SNP12", @"SNP13",
                        @"SNP14",
                        @"SNP15", @"SNP16", @"SNP17", @"SNP18", @"SNP19",
                        @"SNP20",
                        @"SNP21", @"SNP22", @"SNP23", @"SNP24", @"SNP25",
                        @"SNP26",
                        @"SNP27", @"SNP28", @"SNP29", @"SNP30", @"SNP31",
                        @"SNP32",
                        @"SNP33", @"SNP34", @"SNP35", @"SNP36", @"SNP37",
                        @"SNP38",
                        @"SNP39", @"SNP40", @"SNP41", @"SNP42", @"SNP43",
                        @"SNP44",
                        @"SNP45", @"SNP46", @"SNP47", @"SNP48", @"SNP49",
                        @"SNP50",
                        @"E1", @"E2")))
            ...
            writer.Flush();
            var interpretedModel = GetTrainerModel(trainer);
            if (interpretedModel is not null)
            {
                logWriter.WriteLine(interpretedModel);
            }
    ...
    private static object? GetTrainerModel(object trainer)
    {
        return trainer.GetType().GetProperty("Model")?.GetValue(trainer);
    }
}