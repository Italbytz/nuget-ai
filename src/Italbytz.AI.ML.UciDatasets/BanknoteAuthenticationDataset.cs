...
public class BanknoteAuthenticationDataset : Dataset<BanknoteAuthenticationDataset.BanknoteAuthenticationModelInput>
{
    public override bool HasHeader => true;

    public override char Separator => ',';

    public override string FilePrefix => "banknote";

    public override string? LabelColumnName => "class";

    public override ColumnPropertiesV5[] ColumnProperties =>
    [
        new() { ColumnName = "variance", ColumnPurpose = ColumnPurposeType.Feature, ColumnDataFormat = ColumnDataKind.Single, IsCategorical = false },
        new() { ColumnName = "skewness", ColumnPurpose = ColumnPurposeType.Feature, ColumnDataFormat = ColumnDataKind.Single, IsCategorical = false },
        new() { ColumnName = "curtosis", ColumnPurpose = ColumnPurposeType.Feature, ColumnDataFormat = ColumnDataKind.Single, IsCategorical = false },
        new() { ColumnName = "entropy", ColumnPurpose = ColumnPurposeType.Feature, ColumnDataFormat = ColumnDataKind.Single, IsCategorical = false },
    ...
    protected override IEstimator<ITransformer> AdditionalPreprocessingPipeline(MLContext mlContext)
    {
        ...
        {
            new InputOutputColumnPair("variance"),
            new InputOutputColumnPair("skewness"),
            new InputOutputColumnPair("curtosis"),
            new InputOutputColumnPair("entropy")
        });
    }
    ...
    public class BanknoteAuthenticationModelInput
    {
        [LoadColumn(0)] [ColumnName("variance")] public float Variance { get; set; }
        [LoadColumn(1)] [ColumnName("skewness")] public float Skewness { get; set; }
        [LoadColumn(2)] [ColumnName("curtosis")] public float Curtosis { get; set; }
        [LoadColumn(3)] [ColumnName("entropy")] public float Entropy { get; set; }
